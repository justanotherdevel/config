...
var UltiSnippetsProvider = class extends BaseProvider {
  ...
  async showPrompt() {
    let name = import_coc7.workspace.isVim ? `python` : `provider-python`;
    await import_coc7.window.showWarningMessage(`Ultisnips feature of coc-snippets requires python support on vim, check out :h ${name}`, {
      title: "Understand, do'not show again",
      isCloseAffordance: true
    });
    let config = import_coc7.workspace.getConfiguration("snippets.ultisnips", null);
...
` + e.stack;
  ...
  async executePythonCode(pythonCode) {
    if (!this.pythonSupport)
      return;
    try {
      let tmpfile = import_path4.default.join(import_os3.default.tmpdir(), `coc-ultisnips-${uid()}.py`);
